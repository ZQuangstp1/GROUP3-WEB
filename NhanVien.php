...
<body>
    <div id="content">
        ...
        function view_NV()
        {
            ...
            // If a search query is provided, filter the results by the search term
            if (!empty($searchQuery)) {
                // Ensure you use mysqli_real_escape_string() to prevent SQL injection
                $sql = "SELECT * FROM employee WHERE name LIKE '%" . mysqli_real_escape_string($link, $searchQuery) . "%'";
            } else {
                // If no search term is provided, retrieve all records
